...
export function RoundsListPage() {
  ...
  const loadRounds = async () => {
    try {
      setIsLoading(true);
      const response = await api.getRounds(token!);
      const allRounds = response.data || response;

      // Filter only active and scheduled rounds
      const now = new Date().getTime();
      const activeAndScheduled = allRounds.filter((round: Round) => {
        const endTime = new Date(round.endTime).getTime();
        return endTime > now; // Only show rounds that haven't ended yet
      });

      setRounds(activeAndScheduled);
    } catch (err) {
  ...
  const getStatus = (round: Round) => {
    ...
    if (now >= start && now < end) return { text: 'Активен', color: 'text-green-600' };
    return { text: 'Завершён', color: 'text-gray-600' };
  };
      ...
      <div className="w-full max-w-4xl space-y-4">
        ...
        {isAdmin && (
          <div>
            <button
              ...
              className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors duration-150 hover:bg-gray-50"
            >
              Создать раунд
            </button>
          </div>
        ...
        <div className="flex flex-col rounded-lg border border-gray-300 bg-white shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-gray-300 bg-gradient-to-b from-gray-50 to-white px-6 py-4">
            <h1 className="text-lg font-semibold text-gray-700">Список РАУНДОВ</h1>
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-600">{username}</span>
              <button
                onClick={handleLogout}
                className="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 transition-colors duration-150 hover:bg-gray-50"
              >
                Выйти
              </button>
            </div>
          ...
          <div className="space-y-4">
            {rounds.length === 0 ? (
              <p className="text-center text-sm text-gray-500">Нет доступных раундов</p>
            ) : (
              rounds.map((round) => {
